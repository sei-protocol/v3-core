...
describe('UniswapV3Pool', () => {
  ...
  describe('#initialize', () => {
    it('fails if already initialized', async () => {
      ...
      await sleep(3000)
      expect(pool.initialize(encodePriceSqrt(1, 1))).to.be.reverted
    })
    it('fails if starting price is too low', async () => {
      expect(pool.initialize(1)).to.be.revertedWith('R')
      await sleep(3000)
      expect(pool.initialize(MIN_SQRT_RATIO.sub(1))).to.be.revertedWith('R')
    })
    it('fails if starting price is too high', async () => {
      expect(pool.initialize(MAX_SQRT_RATIO)).to.be.revertedWith('R')
      await sleep(3000)
      expect(pool.initialize(BigNumber.from(2).pow(160).sub(1))).to.be.revertedWith('R')
    })
    ...
    it('emits a Initialized event with the input tick', async () => {
      ...
      expect(pool.initialize(sqrtPriceX96)).to.emit(pool, 'Initialize').withArgs(sqrtPriceX96, -6932)
    })
  })
  ...
  describe('#increaseObservationCardinalityNext', () => {
    ...
    //// HAVING ISSUES
    it.only('emits an event including both old and new', async () => {
      const txResp = await pool.initialize(encodePriceSqrt(1, 1))
      await sleep(3000);
  ...
  //         await swapExact0For1(expandTo18Decimals(1), wallet.address)
  //         await sleep(3000);
  //         const txResp = pool.burn(-121200, -120000, liquidityAmount)
  //         await txResp.wait()
  //         await expect(txResp)
  //           .to.emit(pool, 'Burn')
  ...
  ////// NOTE: this test takes super long
  ...
  //   const txResp = await swapExact0For1(3, wallet.address)
  //   await txResp.wait()
  //   await expect(txResp)
  //     .to.emit(token0, 'Transfer')
  ...
  //         const txResp = await flash(1001, 2001, other.address)
  //         await txResp.wait()
  //         await expect(txResp)
  //           .to.emit(pool, 'Flash')
  ...
  //         const txResp = await flash(100, 200, other.address)
  //         await txResp.wait()
  //         await expect(txResp)
  //           .to.emit(token0, 'Transfer')
  ...
  //         const txResp = await flash(101, 0, other.address)
  //         await txResp.wait()
  //         await expect(txResp)
  //           .to.emit(token0, 'Transfer')
  ...
  //         const txResp = await flash(0, 102, other.address)
  //         await txResp.wait()
  //         await expect(txResp)
  //           .to.emit(token1, 'Transfer')
  ...
  //         const txResp = await flash(balance0, balance1, other.address)
  //         await txResp.wait()
  //         await expect(txResp)
  //           .to.emit(token0, 'Transfer')
  ...
  //         const txResp = await flash(0, 0, constants.AddressZero, 567, 0)
  //         await txResp.wait()
  //         await expect(txResp)
  //           .to.emit(token0, 'Transfer')
  ...
  //         const txResp = await flash(0, 0, constants.AddressZero, 0, 678)
  //         await txResp.wait()
  //         await expect(txResp)
  //           .to.emit(token1, 'Transfer')
  ...
  //         const txResp = await flash(0, 0, constants.AddressZero, 789, 1234)
  //         await txResp.wait()
  //         await expect(txResp)
  //           .to.emit(token0, 'Transfer')
  ...
  //       it('increases the fee growth by the expected amount', async () => {
  //         await await flash(2002, 4004, other.address)
  //         await sleep(3000);
  ...
  //     await sleep(3000);
  //     await await flash(0, 0, wallet.address, MaxUint128, MaxUint128)
  //     await sleep(3000);
